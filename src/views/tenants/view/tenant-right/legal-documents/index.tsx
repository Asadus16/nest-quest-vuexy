...
const LegalDocumentsTab = ({ invitation }: { invitation: TenantInvitationType }) => {
  const tenant = invitation.tenant

  const hasEmiratesId = tenant?.emirates_id_number || tenant?.emirates_id_front_url
  const hasPassport = tenant?.passport_number || tenant?.passport_copy_url
    ...
    <Grid container spacing={6}>
      ...
      <Grid size={{ xs: 12, sm: 6 }}>
        <DocumentCard
          ...
          title='Emirates ID'
          fields={[
            { label: 'ID Number', value: tenant?.emirates_id_number },
            {
              label: 'Expiry Date',
              value: tenant?.emirates_id_expiry ? new Date(tenant.emirates_id_expiry).toLocaleDateString() : null
            }
          ]}
        />
      </Grid>
      ...
      <Grid size={{ xs: 12, sm: 6 }}>
        <DocumentCard
          ...
          title='Passport'
          fields={[
            { label: 'Passport Number', value: tenant?.passport_number },
            {
              label: 'Expiry Date',
              value: tenant?.passport_expiry ? new Date(tenant.passport_expiry).toLocaleDateString() : null
            },
            { label: 'Issued Country', value: tenant?.passport_issued_country }
          ]}
        />
      </Grid>

      {tenant?.emirates_id_front_url && (
        <Grid size={{ xs: 12, sm: 6 }}>
          <Card>
