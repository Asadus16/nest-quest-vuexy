...
// Component Imports
import InviteTenantDrawer from './InviteTenantDrawer'
...
// Column Definitions
const columnHelper = createColumnHelper<TenantInvitationType>()
...
const TenantTable = ({
  ...
  invitations: TenantInvitationType[]
  activeTab: string
  onRefresh: () => void
}) => {
  // Hooks
  ...
  const filteredData = useMemo(() => {
    ...
    if (activeTab === 'linked') {
      data = data.filter(inv => inv.status === 'ACCEPTED' || inv.status === 'CREATED')
  ...
  const columns = useMemo<ColumnDef<TenantInvitationType, any>[]>(
    () => [
      {
        id: 'select',
        header: ({ table }) => (
      ...
      columnHelper.accessor('email', {
        header: 'Tenant',
        cell: ({ row }) => {
          const name = row.original.tenant?.full_name || row.original.email.split('@')[0]
      ...
      }),
      {
        ...
        cell: ({ row }) => (
          <div className='flex items-center'>
            <IconButton onClick={() => router.push(`/${locale}/view-tenant/${row.original.id}`)}>
