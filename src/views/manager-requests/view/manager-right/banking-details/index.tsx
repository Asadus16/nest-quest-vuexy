...
const BankingDetailsTab = ({ invitation }: { invitation: ManagerInvitationType }) => {
  const pm = invitation.property_manager

  const hasBankDetails = pm?.bank_name || pm?.account_number || pm?.iban
            ...
            {hasBankDetails ? (
              <Grid container spacing={4}>
                <Grid size={{ xs: 12, sm: 6 }}>
                  <DetailRow icon='tabler-building-bank' label='Bank Name' value={pm?.bank_name} />
                </Grid>
                <Grid size={{ xs: 12, sm: 6 }}>
                  <DetailRow icon='tabler-map-pin' label='Bank Branch' value={pm?.bank_branch} />
                </Grid>
                <Grid size={{ xs: 12, sm: 6 }}>
                  <DetailRow icon='tabler-credit-card' label='Account Number' value={pm?.account_number} />
                </Grid>
                <Grid size={{ xs: 12, sm: 6 }}>
                  <DetailRow icon='tabler-receipt' label='IBAN' value={pm?.iban} />
                </Grid>
              </Grid>
            ) : (
